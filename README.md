...
3. **Set up environment variables**:
   ...
   # Email alert (Buyer Inquiry notifications)
   ...
   ALERT_EMAIL=vinothinis.23it@kongu.edu
   ```
...
## Deployment (Recommended: Split Frontend and Backend)

Use the standard MERN deployment model:
...
Set the backend API base URL for production builds in `frontend/.env.production`:

```env
VITE_API_URL=https://garment-backend-6y2y.onrender.com
```

Then build frontend before deploying:

```bash
npm run build
```

Your React code can keep calling relative API paths through `apiFetch('/api/...')`, which resolves to `VITE_API_URL` in production.

### Backend configuration

Set `CLIENT_URL` on Render to your frontend domain (or multiple domains as comma-separated values):

```env
CLIENT_URL=https://your-frontend.vercel.app,https://your-frontend.netlify.app
```

This enables CORS and credentialed requests from your deployed frontend.
